...
namespace FP.Core.Api.Providers.Providers.Networks.ERC20;

public class CryptoApiErc20Provider : ICryptoApiProvider
{
	private readonly IHttpClientFactory _httpClientFactory;
	private static readonly JsonSerializerOptions Options = new(JsonSerializerDefaults.Web);

	public CryptoApiErc20Provider(IHttpClientFactory httpClientFactory)
	{
	...
	public async Task<CryptoCreatedWallet?> CreateWallet()
	{
		...
		using var response = await httpClient.GetAsync("erc20/create_wallet");
		if (response.StatusCode != HttpStatusCode.OK) return null;
		var a = await response.Content.ReadAsStringAsync();
		return await response.Content.ReadFromJsonAsync<CryptoCreatedWallet>();
	}

	public WalletType Type => WalletType.Erc20;

	public async Task<decimal?> GetWalletBalance(string walletAddress)
	{
		var httpClient = _httpClientFactory.CreateClient("Eth");
		using var response = await httpClient.GetAsync($"erc20/get_balance_usdt/{walletAddress}");
		if (response.StatusCode != HttpStatusCode.OK) return null;
	...
	public async Task<bool> TransferTokenNoFee(Wallet fromWallet, string toWalletaddress, decimal amount)
	{
		try
		{
			...
			using var response = await httpClient.PostAsJsonAsync("erc20/transfer_usdt", transferRequest);
	...
	public async Task<bool> TransferGasToWallet(Wallet fromWallet, string toWalletAddress, decimal amount)
	{
		try
		{
			...
			using var response = await httpClient.PostAsJsonAsync("erc20/transfer_gas", transferRequest);
			return response.StatusCode == HttpStatusCode.OK;
		}
		catch (Exception ex)
		{
